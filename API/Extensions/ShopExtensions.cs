...
public static class ShopExtensions
{
    ...
    public static ShopStats GetStats(this Shop shop)
    {
        ...
        var currentWeek = DateTime.Now.Date.AddDays(-1 * (int) DateTime.Now.DayOfWeek);
        ...
        shopStats.Monthly.ElementData =
            GroupElements(items.Where(i => i.Date.Month == currentMonth && i.Date.Date <= DateTime.Today.Date),
                i => i.Date.Day);
        ...
        shopStats.Weekly.ElementData =
            GroupElements(items.Where(i => i.Date >= currentWeek && i.Date.Date <= DateTime.Today.Date),
                i => i.Date.Day);
        ...
        shopStats.Weekly.TotalPurchases =
            purchases.Where(o => o.Date.Date >= currentWeek && o.Date.Date <= today).Sum(p => p.Total);
        shopStats.Weekly.TotalPurchasesPaid =
            purchases.Where(o => o.Date.Date >= currentWeek && o.Date.Date <= today).Sum(p => p.Paid);
        shopStats.Weekly.TotalSales =
            sales.Where(o => o.Date.Date >= currentWeek && o.Date.Date <= today).Sum(p => p.Total);
        shopStats.Weekly.TotalSalesPaid =
            sales.Where(o => o.Date.Date >= currentWeek && o.Date.Date <= today).Sum(p => p.Paid);
    ...
    private static List<StatElementData> GroupElements(IEnumerable<StatElementData> data,
        Func<StatElementData, int> keySelector)
    {
    ...
    private static List<MoneyTransaction> GenerateFakeData(string shopId)
    {
        ...
        for (var i = 1; i <= currentMonth; i++)
        {
            ...
            for (var j = 1; j <= days; j++)
            {
                ...
                for (var x = 0; x < rand.Next(200); x++)
                {
                    var direction = (TransactionDirection) rand.Next(2);
                    var amount = direction == TransactionDirection.incoming
                        ? rand.Next(2000, 4000)
                        : rand.Next(1000, 2500);
